...
var initPage,
    ...
    sadrzaj1, kategorija, parovi, opcije1, opcije2,
    pogreske = [],
    vrijeme = 0;
...
$(document).ready(function() {
    ...
    function odgovor() {
        ...
        if (document.getElementById("pageBeginCountdown").value == "0" && iskljuci_v == 0) {
            pogreske.push("<strong>" + parovi[1] + "</strong> = " + parovi[0])
            bodovi -= 10;
            $("#zvono")[0].play();
            swal({
            ...
            lista_g=parovi[1].split(" / ")
            if ($("#odgovor").val().toLowerCase() == parovi[1].toLowerCase() || lista_g.includes($("#odgovor").val().toLowerCase())) {
